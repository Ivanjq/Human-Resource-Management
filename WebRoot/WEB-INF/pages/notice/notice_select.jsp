...
<form action="selectNotice" method="post">
    ...
    <c:forEach items="${requestScope.noticeList}" var="notice">
    	<tr>
    		...
    		<td align="center"><a href="../updateNotice?id=${notice.id}">修改</a></td>
    		<td align="center"><a href="../deleteNotice?id=${notice.id}">删除</a></td>
    	</tr>
    </c:forEach>
	</table>
	<table class="tbcss" align="center">
		<tr>
			<c:if test="${requestScope.page.hasPre}">
				<td>
					<a href="selectNotice2?pageNow=1">  首页  </a>
				</td>
			</c:if>
			<c:if test="${requestScope.page.hasPre}">
				<td>
					<a href="selectNotice2?pageNow=${requestScope.page.pageNow-1}">  上一页   </a>
				</td>
			</c:if>
			<c:if test="${requestScope.page.hasNext}">
			<td>
					<a href="selectNotice2?pageNow=${requestScope.page.pageNow+1}">  下一页   </a>
				</td>
			</c:if>
			<c:if test="${requestScope.page.hasNext}">
				<td>
					<a href="selectNotice2?pageNow=${requestScope.page.totalPage}">  尾页  </a>
				</td>
			</c:if>
			<td>
